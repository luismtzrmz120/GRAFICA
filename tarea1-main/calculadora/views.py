...
def datos_grafica(request):
    ...
    res = cur.execute("""SELECT * FROM Game_usuario;""")
    ...
    for item in resultado:
        registro = list(item)
        json.append({"id": registro[0],"grado": int(registro[3]),"num_lista": int(registro[5])})
...
def grafica(request):
    ...
    data = []
    data.append(['id', 'grado', 'num_lista'])
    ...
    for elemento in range(size):
        id = resultados[elemento]['id']
        grado = resultados[elemento]['grado']
        num_lista = resultados[elemento]['num_lista']
        data.append([id, grado, num_lista])

    print(data)
    modified_data = dumps(data)

    JSON = {'datos':modified_data}

    return render(request,'column.html',JSON)
